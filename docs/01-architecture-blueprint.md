...
| -------------------- | ------------------------------------------------------------------------ | ------------------------------- |
| `app/`               | Routes, layouts, metadata, server actions                                | features, components, server    |
| `features/<module>/` | Module UI, hooks, zod schemas, query keys                                | components, lib, domain (types) |
| `components/`        | Design system (`ui/` shadcn), layout, data display, forms                | lib                             |
| `server/`            | Services (use cases), repositories, authz, events. Marked `server-only`. | domain, lib                     |
| `domain/`            | Pure TS: state machines, pricing, BOM math, numbering, money             | nothing framework-specific      |
| `lib/`               | env, prisma, supabase clients, logger, utils                             | —                               |
